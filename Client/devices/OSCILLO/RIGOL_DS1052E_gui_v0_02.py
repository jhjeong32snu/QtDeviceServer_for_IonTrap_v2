...
class DS1052E_GUI(QtWidgets.QWidget, Ui_Form):
    ...
    def ThemeChange(self, theme):
        if theme == 'white':
            ...
            for ch in range(2):
                self.axes[ch].set_facecolor('w')
                self.axes[ch].tick_params(axis='x', colors='k', length=0)
                self.axes[ch].tick_params(axis='y', colors='k', length=0)
                for spine in self.spine_list:
                    self.axes[ch].spines[spine].set_color('k')
                self.setStyleSheet("background-color:rgb(255, 255, 255)")
                for attr in self.Theme_clist:
                    attr.setStyleSheet("color:rgb(0, 0, 0)")
                for attr in self.Theme_alist:
                    attr.setStyleSheet("background-color:rgb(70, 70, 70); color:rgb(255, 255, 255)")
                    
        elif theme == "dark":
            self.fig.set_facecolor(self.rgb_color)
            for ch in range(2):
                self.axes[ch].set_facecolor(self.rgb_color)
                self.axes[ch].tick_params(axis='x', colors='w', length=0)
                self.axes[ch].tick_params(axis='y', colors='w', length=0)
                for spine in self.spine_list:
                    self.axes[ch].spines[spine].set_color('w')
                self.setStyleSheet("background-color:rgb(70, 70, 70)")
                for attr in self.Theme_clist:
                    attr.setStyleSheet("color:rgb(255, 255, 255)")
                for attr in self.Theme_alist:
                    attr.setStyleSheet("background-color:rgb(0, 0, 0); color:rgb(255, 255, 255)")
        self.canvas.draw()
        
    def ChamberChange(self, chamber):
        if chamber == "EA":
            self.ThemeChange('white')
            osc_id = 1
...
if __name__ == "__main__":
    ...
    OSC = DS1052E_GUI(instance_name='DS1052E_v0.01')
