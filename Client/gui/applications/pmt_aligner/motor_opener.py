...
class MotorOpener(QtWidgets.QWidget):
    ...
    def connectSignals(self):
        self.controller._sig_motor_initialized.connect(self.changeProgressBar)
        self.controller._sig_motor_finished_loading.connect(self.finishLoadingMotors)
        
    
    def changeProgressBar(self, curr_idx, max_idx, nickname):
        self.progress_bar.changeProgressBar(curr_idx, max_idx)
        self.progress_bar.changeLabelText("Initiating motors '%s' (%d/%d)..." % (nickname, curr_idx, max_idx))
