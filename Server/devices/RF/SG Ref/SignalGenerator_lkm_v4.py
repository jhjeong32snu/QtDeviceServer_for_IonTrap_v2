...
v2: When commit to DB, Vpp selectively retrieves depending on chambers (EC = BNC, EA = N-type)
v4: Changed 'def output_on_off' to consider device_type
"""
...
new_path_list.append(dirname + '/../Oscilloscope')  # For RIGOL_DS1052E_gui_v0_03_JJH.py
...
from RIGOL_DS1052E_gui_v0_03_JJH import DS1052E_GUI
...
class SignalGenerator(QtWidgets.QWidget, Ui_QWidget):
    ...
    def read_power(self, output_type):
        ...
        if dBm > self.max_dBm[output_type] or dBm < self.min_dBm[output_type]:
            color = 'red'
    ...
    def apply_power(self, output_type):
        ...
        if dBm <= self.max_dBm[output_type] and dBm >= self.min_dBm[output_type]:
    ...
    def load_config(self):
        ...
        if self.device_type == 0:
            self.max_dBm, self.min_dBm = [], []
            self.max_dBm.append(float(self.config.get(
                'SG' + str(self.device_index), 'max_power_dBm_NTYPE')))
            self.max_dBm.append(float(self.config.get(
                'SG' + str(self.device_index), 'max_power_dBm_BNC')))
            
            self.min_dBm.append(float(self.config.get(
                'SG' + str(self.device_index), 'min_power_dBm_NTYPE')))
            self.min_dBm.append(float(self.config.get(
                'SG' + str(self.device_index), 'min_power_dBm_BNC')))
    ...
    def load_config_curr(self):
        ...
        if self.device_type == 0:
            curr_dBm_NTYPE = float(self.config.get(
                'SG' + str(self.device_index),
                'curr_power_dBm_NTYPE'))
            curr_dBm_BNC = float(self.config.get(
                'SG' + str(self.device_index),
                'curr_power_dBm_BNC'))
            self.dBm_spinbox.setValue(curr_dBm_NTYPE)
            self.dBm_updated(0)
            self.dBm_BNC_spinbox.setValue(curr_dBm_BNC)
            self.dBm_updated(1)
        else:
...
class MainWindow(QtWidgets.QDialog):
    ...
    def initUI(self):
        ...
        self.config_frame.setMinimumSize(400, 120)
        self.config_frame.setMaximumSize(400, 120)
        ...
        self.OSC_frame.setMinimumSize(210, 120)
        self.OSC_frame.setMaximumSize(210, 120)
...
#            self.OSC.closeEvent(event)
    ...
    def open_OSC(self):
        ...
        identity = ip.split('.')[-1]
        if   identity == '17' :
            self.OSC.CMB_CHAMBER.setCurrentIndex(1)
        elif identity == "19":
            self.OSC.CMB_CHAMBER.setCurrentIndex(0)
